...
| *escapeString*
| Sets the escape string to be used in the filtering of resources included in the assembly. Expressions preceded with this String won't be interpolated. If you use "\" as the escape string then \${foo} will be replaced with ${foo}.

| *delimiters*
| Sets the delimiters to be used in the filtering of resources included in the assembly. These delimiters are specified in the form 'beginToken*endToken'. If no '*' is given, the delimiter is assumed to be the same for start and end.
